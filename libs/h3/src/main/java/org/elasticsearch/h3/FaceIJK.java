...
final class FaceIJK {
     ...
     */
    public CellBoundary faceIjkPentToCellBoundary(final int res, final int start, final int length) {
        ...
        for (int vert = start; vert < start + length + additionalIteration; vert++) {
            ...
            scratch1.reset(coord[v][0], coord[v][1], coord[v][2]);
            ...
            if (isResolutionClassIII && vert > start) {
                ...
                scratch2.reset(fijk.coord.i, fijk.coord.j, fijk.coord.k);
                // rotate and translate for adjacent face
                for (int i = 0; i < fijkOrient.ccwRot60; i++) {
                    scratch2.ijkRotate60ccw();
                }

                final int unitScale = unitScaleByCIIres[adjRes] * 3;
                scratch2.ijkAdd(fijkOrient.translateI * unitScale, fijkOrient.translateJ * unitScale, fijkOrient.translateK * unitScale);
