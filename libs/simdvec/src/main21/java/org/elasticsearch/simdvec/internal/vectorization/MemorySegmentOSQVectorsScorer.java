...
public final class MemorySegmentOSQVectorsScorer extends OSQVectorsScorer {
    ...
    private static final VectorSpecies<Short> SHORT_SPECIES_64 = ShortVector.SPECIES_64;
    ...
    public MemorySegmentOSQVectorsScorer(IndexInput in, int dimensions, MemorySegment memorySegment) {
        super(in, dimensions);
    ...
    public void int4BitDotProductBulk(byte[] q, int count, float[] scores) throws IOException {
        ...
        if (length >= 16 && PanamaESVectorUtilSupport.HAS_FAST_INTEGER_VECTORS) {
            if (PanamaESVectorUtilSupport.VECTOR_BITSIZE >= 256) {
                int4BitDotProduct256Bulk(q, count, scores);
                return;
            } else if (PanamaESVectorUtilSupport.VECTOR_BITSIZE == 128) {
                int4BitDotProduct128Bulk(q, count, scores);
                return;
            }
        }
        super.int4BitDotProductBulk(q, count, scores);
    }

    private void int4BitDotProduct128Bulk(byte[] q, int count, float[] scores) throws IOException {
    ...
    private void int4BitDotProduct256Bulk(byte[] q, int count, float[] scores) throws IOException {
        for (int iter = 0; iter < count; iter++) {
            long subRet0 = 0;
            long subRet1 = 0;
            long subRet2 = 0;
    ...
    public void quantizeScoreBulk(byte[] q, OptimizedScalarQuantizer.QuantizationResult queryCorrections,
        ...
        if (length >= 16 && PanamaESVectorUtilSupport.HAS_FAST_INTEGER_VECTORS) {
            if (PanamaESVectorUtilSupport.VECTOR_BITSIZE >= 256) {
                quantizeScore256Bulk(q, queryCorrections, similarityFunction, centroidDp, scores);
                return;
            } else if (PanamaESVectorUtilSupport.VECTOR_BITSIZE == 128) {
                quantizeScore128Bulk(q, queryCorrections, similarityFunction, centroidDp, scores);
                return;
            }
        }
        super.quantizeScoreBulk(q, queryCorrections, similarityFunction, centroidDp, scores);
    }

    private void quantizeScore128Bulk(byte[] q, OptimizedScalarQuantizer.QuantizationResult queryCorrections,
                                      VectorSimilarityFunction similarityFunction,
                                      float centroidDp, float[] scores) throws IOException {
        int4BitDotProduct128Bulk(q, BULK_SIZE, scores);
        ...
        for (; i < limit; i += FLOAT_SPECIES_128.length()) {
            var ax =
                FloatVector.fromMemorySegment(FLOAT_SPECIES_128, memorySegment, offset + i * 4, ByteOrder.LITTLE_ENDIAN);
            var lx =
                FloatVector.fromMemorySegment(FLOAT_SPECIES_128, memorySegment, offset + 4 * BULK_SIZE + i * 4, ByteOrder.LITTLE_ENDIAN)
                    .sub(ax);
            var targetComponentSums =
                ShortVector.fromMemorySegment(SHORT_SPECIES_128, memorySegment, offset + 8 * BULK_SIZE + i * 2, ByteOrder.LITTLE_ENDIAN)
                    .convert(VectorOperators.S2I, 0).reinterpretAsInts().and((int) 0xffff).convert(VectorOperators.I2F, 0);
            var additionalCorrections =
                FloatVector.fromMemorySegment(FLOAT_SPECIES_128, memorySegment, offset + 10 * BULK_SIZE + i * 4, ByteOrder.LITTLE_ENDIAN);
    ...
    private void quantizeScore256Bulk(byte[] q, OptimizedScalarQuantizer.QuantizationResult queryCorrections,
                                      VectorSimilarityFunction similarityFunction,
                                      float centroidDp, float[] scores) throws IOException {
        int4BitDotProduct256Bulk(q, BULK_SIZE, scores);
        ...
        for (; i < limit; i += FLOAT_SPECIES_256.length()) {
            var ax =
                FloatVector.fromMemorySegment(FLOAT_SPECIES_256, memorySegment, offset + i * 4, ByteOrder.LITTLE_ENDIAN);
            var lx =
                FloatVector.fromMemorySegment(FLOAT_SPECIES_256, memorySegment, offset + 4 * BULK_SIZE + i * 4, ByteOrder.LITTLE_ENDIAN)
                    .sub(ax);
            var targetComponentSums =
                ShortVector.fromMemorySegment(SHORT_SPECIES_256, memorySegment, offset + 8 * BULK_SIZE + i * 2, ByteOrder.LITTLE_ENDIAN)
                    .convert(VectorOperators.S2I, 0).reinterpretAsInts().and((int) 0xffff).convert(VectorOperators.I2F, 0);
            var additionalCorrections =
                FloatVector.fromMemorySegment(FLOAT_SPECIES_256, memorySegment, offset + 10 * BULK_SIZE + i * 4, ByteOrder.LITTLE_ENDIAN);
