            ...
            while (token != XContentParser.Token.END_OBJECT) {
                ...
                if (fieldName.equals(SKETCH_FIELD.getPreferredName())) {
                    ...
                    while (token != XContentParser.Token.END_ARRAY) {
                        ...
                        int val = subParser.intValue();
                        if (val < 0) {
                            throw new MapperParsingException("error parsing field ["
                                + name() + "], ["+ SKETCH_FIELD + "] elements must be >= 0 but got " + val);
                        }
                        if (val > Byte.MAX_VALUE) { // is that correct? what is the real max value for a runLen?
                            throw new MapperParsingException("error parsing field ["
                                + name() + "], ["+ SKETCH_FIELD + "] elements must be <= " + Byte.MAX_VALUE + " but got " + val);
                        }
                        runLens.add((byte) val);
                        token = subParser.nextToken();
                    }
                } else {
                    throw new MapperParsingException("error parsing field [" +
